...
認証情報、セッション、ライブラリ、蔵書、ページ、読書位置、コレクション、ジョブは、一つの`bookcafe.sqlite`へ保存します。
...
コレクションは、ユーザーが手動で本をまとめるプレイリスト形式の機能です。コレクションはユーザーとライブラリに属し、異なるライブラリの本を混在させることはできません。

コレクション内の本は手動で並び替えられます。アーカイブ済みまたはmissing状態の本はコレクションから非表示になりますが、所属情報は保持します。原本が復帰するかアーカイブから戻すと、再び表示されます。削除された本の所属情報は自動的に削除します。
...
Desktop ManagerはHono serverの起動、停止、状態確認、Web UIの起動、ログフォルダーの表示を担当します。アカウント、ライブラリ、スキャン、ネットワーク、サムネイルの設定はWeb UIで行います。
